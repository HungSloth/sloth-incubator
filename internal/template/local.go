...
`,
		"files/.incubator/preview/config.yaml.tmpl": `enabled: true
app_command: "echo \"Set preview app_command in .incubator/preview/config.yaml\" && sleep infinity"
novnc_port: 6080
vnc_port: 5900
`,
		"files/.incubator/preview/entrypoint.sh": `#!/usr/bin/env bash
set -euo pipefail
...
wait "${XVFB_PID}"
`,
		"files/.incubator/preview/Dockerfile": `FROM ubuntu:24.04
